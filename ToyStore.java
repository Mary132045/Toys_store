...
class ToyStore {
    ...
    public void play() {
        ...
        if (sum == 0) {
            System.out.println("Игрушек нет в наличии. Розыгрыш невозможен");
            System.out.println("Добавьте новые игрушки или исправьте количество");
            return;
        }
        sum = 0;
        ...
        if (!prizePool.isEmpty()) {
            ...
            System.out.println("Поздравляем! Ваш приз:" + wonToy.getName());
            wonToy.setQuantity(wonToy.getQuantity() - 1);
            // Запись выигранной игрушки в файл
            writeToFile(wonToy);
        } else {
            System.out.println("Вам обязательно повезёт в другой раз!");
        }
    }
